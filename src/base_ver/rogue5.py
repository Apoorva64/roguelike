...
class Map(object):
    ...
    def move(self, e, way):
        ...
        dest = orig + way
        if dest in self:
            if self.get(dest) == self.ground:
                self._mat[orig.y][orig.x] = self.ground
                self._mat[dest.y][dest.x] = e
                self._elem[e] = dest
            elif self.get(dest) != self.empty and self.get(dest).meet(e) and self.get(dest) != self.hero:
                self.rm(dest)
...
class Game(object):
    ...
    equipments = {0: [Equipment("potion", "!", usage=heal), Equipment("gold", "o")],
                  1: [Equipment("sword"), Equipment("bow"), Equipment("potion", "!", usage=teleport)],
                  2: [Equipment("chainmail")], 3: [Equipment("portoloin", "w", usage=teleport)]}
    """ available monsters """
    ...
    def randEquipment(self):
        ...
        return self.randElement(Game.equipments) + 100

    def randMonster(self):
        """Returns a random monster."""
        return self.randElement(Game.monsters) + 1000
...
p = list(filter(lambda x: x.name == 'potion', Game.equipments[0]))[0]
c = Creature("Goblin", 5)
print(p.use(c))
print(c.description())
