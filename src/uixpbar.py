...
class UIScreenSpaceXpBar(UIElement):
    """
    A UI that will display xp capacity and current xp for a sprite in 'screen space'.
    That means it won't move with the camera. This is a good choice for a user/player sprite.

    :param relative_rect: The rectangle that defines the size and position of the xp bar.
    :param manager: The UIManager that manages this element.
    :param sprite_to_monitor: The sprite we are displaying the xp of.
    ...
    def __init__(self,
        ...
        self._create_valid_ids(container=container,
                               ...
                               object_id=object_id,
                               element_id='screen_space_xp_bar')

        self.current_xp = 50
        self.xp_capacity = 100
        self.xp_percentage = self.current_xp / self.xp_capacity
        ...
        if sprite_to_monitor is not None:
            if not hasattr(sprite_to_monitor, 'xp_capacity'):
                raise AttributeError('Sprite does not have xp_capacity attribute')
            if not hasattr(sprite_to_monitor, 'current_xp'):
                raise AttributeError('Sprite does not have current_xp attribute')
    ...
    def set_sprite_to_monitor(self, sprite_to_monitor: pygame.sprite.Sprite):
        """
        Sprite to monitor the xp of. Must have 'xp_capacity' and 'current_xp'
        attributes.
    ...
    def rebuild(self):
        """
        Rebuild the xp bar entirely because the theming data has changed.
        ...
        self.current_xp_rect = pygame.Rect((self.shadow_width + self.border_width,
                                            self.shadow_width + self.border_width),
                                           (int(self.capacity_width * self.xp_percentage),
    ...
    def redraw(self):
        """
        Redraws the xp bar rectangles and text onto the underlying sprite's image surface.
        Takes a little while so we only do it when the xp has changed.
        """
        xp_display_string = f'level: {self.sprite_to_monitor.level} xp points: ' + str(
            int(self.current_xp)) + "/" + str(int(self.xp_capacity))
        ...
        theming_parameters = {'normal_bg': self.bar_unfilled_colour,
                              ...
                              'filled_bar_width_percentage': self.xp_percentage,
                              'font': self.font,
                              'text': xp_display_string,
    ...
    def update(self, time_delta: float):
        """
        Updates the xp bar sprite's image with the latest xp data from the
        sprite we are monitoring. Only triggers a rebuild if the xp values have changed.
        ...
        if (self.alive() and self.sprite_to_monitor is not None and
                (self.sprite_to_monitor.xp_capacity != self.xp_capacity or
                 self.current_xp != self.sprite_to_monitor.current_xp)):
            self.current_xp = self.sprite_to_monitor.current_xp
            self.xp_capacity = self.sprite_to_monitor.xp_capacity
            self.xp_percentage = self.current_xp / self.xp_capacity

            rect_width = int(self.capacity_width * self.xp_percentage)
            self.current_xp_rect = pygame.Rect((self.shadow_width + self.border_width,
                                                self.shadow_width + self.border_width),
                                               (rect_width,
                                                self.capacity_height))
