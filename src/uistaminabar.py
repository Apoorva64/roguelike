...
class UIScreenSpaceStaminaBar(UIElement):
    """
    A UI that will display stamina capacity and current stamina for a sprite in 'screen space'.
    That means it won't move with the camera. This is a good choice for a user/player sprite.

    :param relative_rect: The rectangle that defines the size and position of the stamina bar.
    :param manager: The UIManager that manages this element.
    :param sprite_to_monitor: The sprite we are displaying the stamina of.
    ...
    def __init__(self,
        ...
        self._create_valid_ids(container=container,
                               ...
                               object_id=object_id,
                               element_id='screen_space_stamina_bar')

        self.current_stamina = 50
        self.stamina_capacity = 100
        self.stamina_percentage = self.current_stamina / self.stamina_capacity
        ...
        self.current_stamina_rect = None
        ...
        if sprite_to_monitor is not None:
            if not hasattr(sprite_to_monitor, 'stamina_capacity'):
                raise AttributeError('Sprite does not have stamina_capacity attribute')
            if not hasattr(sprite_to_monitor, 'current_stamina'):
                raise AttributeError('Sprite does not have current_stamina attribute')
    ...
    def set_sprite_to_monitor(self, sprite_to_monitor: pygame.sprite.Sprite):
        """
        Sprite to monitor the stamina of. Must have 'stamina_capacity' and 'current_stamina'
        attributes.
    ...
    def rebuild(self):
        """
        Rebuild the stamina bar entirely because the theming data has changed.
        ...
        self.current_stamina_rect = pygame.Rect((self.shadow_width + self.border_width,
                                                 self.shadow_width + self.border_width),
                                                (int(self.capacity_width * self.stamina_percentage),
    ...
    def redraw(self):
        """
        Redraws the stamina bar rectangles and text onto the underlying sprite's image surface.
        Takes a little while so we only do it when the stamina has changed.
        """
        stamina_display_string = 'stamina points: ' + str(int(self.current_stamina)) + "/" + str(
            int(self.stamina_capacity))
        ...
        theming_parameters = {'normal_bg': self.bar_unfilled_colour,
                              ...
                              'filled_bar_width_percentage': self.stamina_percentage,
                              'font': self.font,
                              'text': stamina_display_string,
    ...
    def update(self, time_delta: float):
        """
        Updates the stamina bar sprite's image with the latest stamina data from the
        sprite we are monitoring. Only triggers a rebuild if the stamina values have changed.
        ...
        if (self.alive() and self.sprite_to_monitor is not None and
                (self.sprite_to_monitor.stamina_capacity != self.stamina_capacity or
                 self.current_stamina != self.sprite_to_monitor.current_stamina)):
            self.current_stamina = self.sprite_to_monitor.current_stamina
            self.stamina_capacity = self.sprite_to_monitor.stamina_capacity
            self.stamina_percentage = self.current_stamina / self.stamina_capacity

            rect_width = int(self.capacity_width * self.stamina_percentage)
            self.current_stamina_rect = pygame.Rect((self.shadow_width + self.border_width,
                                                     self.shadow_width + self.border_width),
                                                    (rect_width,
                                                     self.capacity_height))
