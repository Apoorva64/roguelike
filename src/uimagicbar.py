...
class UIScreenSpaceMagicBar(UIElement):
    """
    A UI that will display magic capacity and current magic for a sprite in 'screen space'.
    That means it won't move with the camera. This is a good choice for a user/player sprite.

    :param relative_rect: The rectangle that defines the size and position of the magic bar.
    :param manager: The UIManager that manages this element.
    :param sprite_to_monitor: The sprite we are displaying the magic of.
    ...
    def __init__(self,
        ...
        self._create_valid_ids(container=container,
                               ...
                               object_id=object_id,
                               element_id='screen_space_magic_bar')

        self.current_magic = 50
        self.magic_capacity = 100
        self.magic_percentage = self.current_magic / self.magic_capacity
        ...
        if sprite_to_monitor is not None:
            if not hasattr(sprite_to_monitor, 'magic_capacity'):
                raise AttributeError('Sprite does not have magic_capacity attribute')
            if not hasattr(sprite_to_monitor, 'current_magic'):
                raise AttributeError('Sprite does not have current_magic attribute')
    ...
    def set_sprite_to_monitor(self, sprite_to_monitor: pygame.sprite.Sprite):
        """
        Sprite to monitor the magic of. Must have 'magic_capacity' and 'current_magic'
        attributes.
    ...
    def rebuild(self):
        """
        Rebuild the magic bar entirely because the theming data has changed.
        ...
        self.current_magic_rect = pygame.Rect((self.shadow_width + self.border_width,
                                               self.shadow_width + self.border_width),
                                              (int(self.capacity_width * self.magic_percentage),
    ...
    def redraw(self):
        """
        Redraws the magic bar rectangles and text onto the underlying sprite's image surface.
        Takes a little while so we only do it when the magic has changed.
        """
        magic_display_string = 'magic points: '+str(int(self.current_magic)) + "/" + str(int(self.magic_capacity))
        ...
        theming_parameters = {'normal_bg': self.bar_unfilled_colour,
                              ...
                              'filled_bar_width_percentage': self.magic_percentage,
                              'font': self.font,
                              'text': magic_display_string,
    ...
    def update(self, time_delta: float):
        """
        Updates the magic bar sprite's image with the latest magic data from the
        sprite we are monitoring. Only triggers a rebuild if the magic values have changed.
        ...
        if (self.alive() and self.sprite_to_monitor is not None and
                (self.sprite_to_monitor.magic_capacity != self.magic_capacity or
                 self.current_magic != self.sprite_to_monitor.current_magic)):
            self.current_magic = self.sprite_to_monitor.current_magic
            self.magic_capacity = self.sprite_to_monitor.magic_capacity
            self.magic_percentage = self.current_magic / self.magic_capacity

            rect_width = int(self.capacity_width * self.magic_percentage)
            self.current_magic_rect = pygame.Rect((self.shadow_width + self.border_width,
                                                   self.shadow_width + self.border_width),
                                                  (rect_width,
                                                   self.capacity_height))
